...
public class WorkhorseService {
     ...
     * @param from only executions that were created after this timestamp are considered. If <code>null</code>, the default value is genarated by subtracting 24
     *        hours from the {@code LocalDateTime} parameter {@code to}.
     * @param to only executions that were created before this timestamp are considered. If <code>null</code>, the default value is the the current date-time
