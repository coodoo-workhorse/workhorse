...
public class JobScheduler {
    ...
    public void startScheduler() {
        for (Job job : jobEngineController.getAllScheduledJobs()) {
            start(job);
        }
    }
    ...
    public void start(Job job) {
        ...
        if (JobStatus.ACTIVE.equals(job.getStatus()) && job.getSchedule() != null && !job.getSchedule().isEmpty()) {
            ...
            try {
                ...
                jobErrorEvent.fire(new JobErrorEvent(e, JobEngineErrorType.INVALID_SCHEDULE.getMessage(), job.getId(), JobStatus.ERROR));
            }
        }
    }
